...
class StickDragWidget(Widget):
    ...
    def update_sticks(self, *args):
        ...
        with self.canvas:
            ...
            for i in range(self.total_sticks):
                ...
                x = start_x + i * (stick_width + spacing)
                Rectangle(pos=(x, start_y), size=(stick_width, stick_height))

            if self.total_sticks > 0:
                Color(0.3, 0.3, 0.3, 1)
                split_x = start_x + self.split_index * (stick_width + spacing) - spacing / 2
                Rectangle(pos=(split_x, start_y - 10), size=(2, stick_height + 20))
    ...
    def update_split_index(self, x):
        ...
        if relative_x < 0:
            ...
            index = int(relative_x / (stick_width + spacing)) + 1
            self.split_index = max(1, min(index, self.total_sticks - 1))
...
class NumberScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.input1 = TextInput(hint_text='第一个数字', font_size=16, input_filter='int', font_name=chinese_font)
        input_layout.add_widget(self.input1)
        
        self.input2 = TextInput(hint_text='第二个数字', font_size=16, input_filter='int', font_name=chinese_font)
        input_layout.add_widget(self.input2)
        
        self.input3 = TextInput(hint_text='第三个数字', font_size=16, input_filter='int', font_name=chinese_font)
    ...
    def do_divination(self, instance):
        try:
            ...
            if num1 == 0 or num2 == 0 or num3 == 0:
                self.result_label.text = '请输入三个非零整数'
                return
